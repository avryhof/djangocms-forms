...
class FormFieldInlineForm(forms.ModelForm):
    def clean(self):
        ...
        if cleaned_data.get("field_type") in requires_choice_values and not cleaned_data.get("choice_values"):
    ...
    def clean(self):
        ...
        if redirect_delay and not any([page_redirect, external_redirect]):
            self._errors["redirect_delay"] = self.error_class(
                [_("You must specify either a page or external redirect when " "adding a redirect delay.")]
            )
...
class FormBuilder(forms.Form):
    ...
    def prepare_text(self, field):
        ...
        field_attrs.update({"widget": forms.TextInput(attrs=widget_attrs)})
    ...
    def prepare_textarea(self, field):
        ...
        field_attrs.update({"widget": forms.Textarea(attrs=widget_attrs)})
        return forms.CharField(**field_attrs)
    ...
    def prepare_checkbox(self, field):
        ...
        field_attrs.update({"widget": forms.CheckboxInput(attrs=widget_attrs)})

        if field.initial:
            field_attrs.update({"initial": self.to_bool(field.initial)})
    ...
    def prepare_checkbox_multiple(self, field):
        ...
        if field.initial:
            field_attrs.update({"initial": self.split_choices(field.initial)})
    ...
    def prepare_select(self, field):
        ...
        field_attrs.update({"widget": forms.Select(attrs=widget_attrs)})
        ...
        if field.choice_values:
            ...
            field_attrs.update({"choices": choice_list})
    ...
    def prepare_file(self, field):
        ...
        field_attrs.update({"widget": forms.ClearableFileInput(attrs=widget_attrs)})

        if field.choice_values:
            regex = re.compile("[\s]*\n[\s]*")
            choices = regex.split(field.choice_values)
            field_attrs.update({"allowed_file_types": [i.lstrip(".").lower() for i in choices]})
    ...
    def prepare_url(self, field):
        ...
        field_attrs.update({"widget": forms.URLInput(attrs=widget_attrs)})
    ...
    def email_submission(self, form_data, request, referrer):
        ...
        mail_subject = self.form_definition.email_subject or "Form Submission - %s" % self.form_definition.name
        context = {
        ...
        if self.form_definition.email_uploaded_files:
            for field, filedata in list(self.files.items()):
                filedata.open("rb")
...
class SubmissionExportForm(forms.Form):
    ...
    form = forms.ModelChoiceField(
        ...
        label=_("Select a Form"),
        error_messages={"required": _("Please select a form.")},
        help_text=_(
            "Select the form you would like to export entry data from. "
